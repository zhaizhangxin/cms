$(function() {
	...
	//框架信息
	function onlookers(aid, fn ,dev) {
		var url = "/api/activity/"+dev+"/onlookers/" + aid + "";
		$.ajax({
			type: "get",
			headers: {
				rsid: rsidDate
	...
	function onlookersShow(data) {
		...
		if(status == '0') {
			...
			if(!/\.(mp4)$/.test(headPic)) {
				...
				$('#homePic').css('background-image', 'url(' + headPic + ')');
			} else {
			...
			if(!/\.(mp4)$/.test(indexPic)) {
				$('#playVideos').hide();
				$('#pause').hide();
				$('#detailsPic').css('background-image', 'url(' + indexPic + ')');
			} else {
				$('#pause').show();
				$('#playVideos').show();
				$('#playVideos').attr('src', indexPic);
				$('#sources').attr('src', indexPic);
				//点击播放视频
				var myVideo = document.getElementById("playVideos");
			...
			//参与人员
			var users = bodys.users;
			users.forEach(function(item, index) {
				//头像
				var headPic = item.headPic;
				$('.participant').append('<img src="' + headPic + '" alt="" />');
			})
		} else {
			alert(msg);
		}
	...
	$(function() {
		onlookers(aid, onlookersShow,dev);
			...
			comments.forEach(function(item, index) {
				...
				if(commentType == '1') {
					...
					$('body').delegate('#delete' + organizersIndex + '', 'click', function() {
						...
						console.log(cid)
						var parents = $(this).parent().remove();
			//			deleteGraphic(cid);
//						console.log(parents);
					})
					organizersIndex++;
				...
				if(commentType == '2') {
					...
					var cid = item.cid;
					$('.all').append('<div class="graphic" id="graphic' + organizersIndex + '"></div>');
					$('#graphic' + organizersIndex + '').append('<div class="delete" id="delete' + organizersIndex + '" data-cid="' + cid + '"><span>删除</span></div><div class="nameDetails"><div id="profileHead" class="profileHead' + organizersIndex + '"></div><div class="profileName"><p id="profilename" class="profilename' + organizersIndex + '"></p><p id="summary" class="summary' + organizersIndex + '"></p></div></div><div id="weui_panel"><div class="my-gallery" id="details' + organizersIndex + '" itemscope></div></div><div class="thumbsUp"><img src="images/btn_like_normal.png" alt="" /><div class="Thumbs" id="Thumbs' + organizersIndex + '"></div><span id="ThumbsNum" class="ThumbsNum' + organizersIndex + '">' + likeNum + '</span></div><p class="commentNum">评论共<span id="commentNum"></span>条&nbsp;<span id="view" class="view' + organizersIndex + '">查看</span></p>');
...
//						图文
						if(graphicType == '0'){
							pics.forEach(function(item, picIndex) {
								$('#details' + organizersIndex + '').append('<img id="graphicImg" width="100%" height="400px" src="' + item + '">');
//								
							})
						}
//						九宫格
...
//						console.log(parents);
	...
	//删除图文
	function deleteGraphic(cid) {
		$.ajax({
			type: "post",
			headers: {
				rsid: rsidDate
			},
			url: "/api/activity/deleteGraphic/" + cid + "",
			async: true,
			success: function(data) {
				console.log(data)
			},
