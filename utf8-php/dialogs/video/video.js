	...
	function insertUpload() {
		...
		for(var key in uploadVideoList) {
			...
			videoObjs.push({
				...
				height: height,
				align: align,
				videoImage:uploadDir + file.videoImage
			});
		}
...
//				actionUrl = editor.getActionUrl(editor.getOpt('videoActionName')),
				...
				actionUrl = "/upload/imageCategory/"+uid+"/activity/1",
...
//			新增代码
			uploader.options.header.rsid = rsidDate;
			uploader.options.async = false;
...
//							url = utils.formatUrl(actionUrl + (actionUrl.indexOf('?') == -1 ? '?' : '&') + 'encode=utf-8&' + params);
							url = utils.formatUrl(actionUrl+params);
